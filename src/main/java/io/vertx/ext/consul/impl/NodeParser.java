...
class NodeParser {
  ...
  static Node parse(JsonObject json) {
    ...
    Map<String, String> taggedAddressesMap = mapOfStringsFromJsonObject(json.getJsonObject(TAGGED_ADDRESSES_KEY));
    if (!taggedAddressesMap.isEmpty()) {
      node.setTaggedAddresses(taggedAddressesMap);
    }
